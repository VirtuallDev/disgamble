...
import { socket, socketRequest } from '../../apiHandler';
...
const Voice = () => {
  ...
  const sendOffer = async () => {
    if (peerConnection && !peerConnection.localDescription) {
      ...
      socketRequest('webrtc:offer', offer, userId);
    }
  };
  ...
  useEffect(() => {
    ...
    peerConnection.onicecandidate = (event) => {
      if (event.candidate) {
        socketRequest('webrtc:icecandidate', event.candidate, userId);
      }
    };
    ...
    socket.on('webrtc:offer', async (offer, id) => {
      if (userId !== id) {
        ...
        socketRequest('webrtc:answer', answer, userId);
      }
    });
